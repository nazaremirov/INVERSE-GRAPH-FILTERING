...
lmx=max(abs(eig(A)));
alpha_gtvr=1;
G_gtvr=inv(eye(N,N)+alpha_gtvr*(eye(N,N)-A/lmx)^2);
...
SNR_gtvr=-20*log10(norm(X-X_gtvr,'fro')/Norm_X);
...
avg_snr_gtvr{qq}=sum_snr_gtvr/trial;
...
avg_X_gtvr{qq}=sum_X_gtvr/trial;
...
table_gtvr=[avg_snr_gtvr{1}; avg_snr_gtvr{2}; avg_snr_gtvr{3}];

%save('Denoised_US_hourly_Data_for_eta5','Methods_order','table_input','table_iopa','table_icpa','table_gradient','table_centralized','avg_X_icpa','avg_X_iopa','avg_X_gradient','avg_X_centralized','avg_noise','param_alpha_beta','X')
